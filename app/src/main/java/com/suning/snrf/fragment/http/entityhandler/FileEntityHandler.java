 ...
 */
package com.suning.snrf.fragment.http.entityhandler;
...
public class FileEntityHandler {
	...
	public Object handleEntity(HttpEntity entity, EntityCallBack callback, String target, boolean isResume) throws IOException {
		...
		while (!mStop && !(current >= count) && ((readLen = input.read(buffer,0,buffer.length)) != -1) ) {//未全部读取
			os.write(buffer, 0, readLen);
			current += readLen;
			Log.e("", "current:"+current+"   count:"+count + "   mStop:" + mStop + "   input:" + entity.getContentLength());
