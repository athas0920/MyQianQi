 ...
 */
  public void registerPluginForMimeType(String[] indexFiles, String mimeType, WebServerPlugin plugin, Map<String,String> commandLineOptions){
 ...
 */
  ...
  public Response serve(IHTTPSession session){
    ...
    for (    File homeDir : getRootDirs()) {
      if (!homeDir.isDirectory()) {
        return createResponse(Response.Status.INTERNAL_ERROR, NanoHTTPD.MIME_PLAINTEXT,"INTERNAL ERRROR: given path is not a directory (" + homeDir + ").");
      }
    }
  ...
  private Response respond(Map<String,String> headers, String uri){
    ...
    return response != null ? response : createResponse(Response.Status.NOT_FOUND, NanoHTTPD.MIME_PLAINTEXT,"Error 404, file not found.");
  }
...
;
        ...
        if (etag.equals(header.get("if-none-match")))         res=createResponse(Response.Status.NOT_MODIFIED,mime,"");
 else {
