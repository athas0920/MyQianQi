 ...
 */
package com.suning.snrf.fragment.http;

import android.os.SystemClock;
import android.text.TextUtils;

import com.suning.snrf.fragment.FinalHttp;
import com.suning.snrf.fragment.core.AsyncTask;
import com.suning.snrf.fragment.http.entityhandler.EntityCallBack;
import com.suning.snrf.fragment.http.entityhandler.FileEntityHandler;
import com.suning.snrf.fragment.http.entityhandler.StringEntityHandler;
...
public class HttpHandler<T> extends AsyncTask<Object, Object, Object> implements
	...
	private void makeRequestWithRetries(HttpUriRequest request)
		...
		while (retry) {
			try {
				if (!isCancelled()) {
//					HttpResponse response = SNInstrumentation.execute(client,
//							request, context);
					if (!isCancelled()) {
//						handleResponse(response);
		...
		if (executionCount > FinalHttp.getMaxRetries() && !retry)
			publishProgress(UPDATE_FAILURE, new SocketTimeoutException(), 0,
					"Retries out!");
		if (cause != null)
			throw cause;
		else
			...
			try {
				...
				if (entity != null) {
					...
					if (!TextUtils.isEmpty(responseBody.toString())) {
						if (responseBody.toString().contains("Content-Type")) {
							publishProgress(UPDATE_FAILURE, new Exception(), 0,
									"");
						} else {
